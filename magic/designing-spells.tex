...
\pageref{skill:spellcrafting}). All spells in the spell list where
...
\begin{itemize}
    \item \textbf{Melee:} Choosing "Melee" has no effect on mana cost. A
        melee spell applies its effects to a target in melee range if the
        caster succeeds on a skill check of the spell's school contested against
        the target's defence check. Spells with a melee target do not provoke
        opportunity attacks.
    \item \textbf{Missile:} "Missile" is chosen with a range in feet. For every
        5 ft. of range including the first, a missile spell increases
        the mana cost by ½. A missile spell applies its effects to a
        target within the specified range if the caster succeeds on a
        skill check of the spell's school contested against the
        target's defence check.
        % NB: The design intention of including the first five feet in
        % the range cost calculation is to encourage neat multiples of
        % 10 for range, and to add a slight extra premium over melee
        % so it's a bigger decision to take a spell ranged (otherwise in
        % some situations, it would be effectively free if you are
        % already on e.g. 2.5 cost, if the first 5 feet was free, you
        % might as well make it a 10 ft. range spell just to use up the
        % extra 0.5 cost that would get rounded up anyway)
    ...
    \item \textbf{Chain:} "Chain" is chosen with a range in feet, and a number
        of targets. Every 5 ft. of range increases the mana cost by ¼, and the
        ...
        closest creature or similarly conductive target - with preference
        to creatures that haven't been hit yet and ties broken
        randomly - and acts like a missile spell again. This repeats for the
        number of targets chosen. If a target makes a successful
        defence roll, the chain attack forfeits that target, but still
...
\begin{itemize}
    ...
    \item \textbf{Ranged:} "Ranged" is chosen with a range in feet. For every
        5 ft. of range including the first, a ranged spell increases
        its mana cost by ½. A ranged spell applies its effects to a
    ...
    \item \textbf{Area:} "Area" is chosen with a shape (cone, sphere, or cube)
        ...
        by the diameter (for spheres) or length of the shape in 5 ft. increments.
