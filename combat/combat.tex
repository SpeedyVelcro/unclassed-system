...
\begin{multicols*}{2}
    ...
    \section{health}\label{health points}\index{health points}\label{maximum health points}\index{maximum health points}
    Your survivability in combat is represented by your health points
    and your maximum health. % TODO: Elaborate
    ...
    % TODO: area attacks (massive penalties to dodge when further away
    % from the edge, block only mitigates damage, cannot parry)
    ...
    Spells are a special case. Usually starting the casting of a spell
    provokes an opportunity attack, but there are exceptions based on a
    spell's functionality. Spells with melee or touch range do not
    provoke an opportunity attack, unless the caster casts it on
    themselves. Spells that are cast across multiple turns (e.g. spells
    with a one-round casting time) always provoke an opportunity
    attack.
